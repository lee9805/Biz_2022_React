...
const DietItem = ({ food }) => {
  /*
    식품명 표시된곳을 click 했을때 input box가 나타나도록 하려고 한다
    1. DietList 에서 isEdit state 변수를 만들고 item 을 생성하면서
        isEdit 변수의 상ㅌ태에 따라 input box 또는 그냥 문자열이 나타 나도록
        하였다
        ==> 모든 item 이 isEdit 상태를 공유하게 되어 한개의 항목을 클릭하면
        모든 항목에 input box 가 나타났다

    2. DietItem 을 별도로 분리하고 DietItem 에서 isEdit 상태 변수를 생성한 후
        같은 코드로 input box 가 나타나도록 하였다
        똑같은 이름의 isEdit 이지만 각 item 마다 서로 다른 isEdit 만들게 되어
        클릭한 item 만 input box가 나타나도록 변경 된다
    */
  ...
  const removeEvent = (e, d_id) => {
    ...
    // 그중 confirm 함수도 자체지원이 되지 않아 brower 의 함수를
